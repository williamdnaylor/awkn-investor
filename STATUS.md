...
| Vercel production | `awkn-investor.vercel.app` | **deployed 2026-08-06 from `main` @ `ad3ed36`**, deployment `dpl_73bhvUzCt2BWBR4Jdxk6eyTwpacd`; anything on `main` after that commit is documentation only and changes nothing served. Favicon, apple icon and the OG card all serve 200 and `og:image` resolves absolute. **Fully gated:** the root, both decks and their images all 307 an anonymous visitor to `/login?next=…`; only `/login`, `/signup`, `/forgot-password`, `/reset-password` and `/api/auth/*` answer signed-out |
